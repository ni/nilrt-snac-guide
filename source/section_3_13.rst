...
.. _3-13-17--internal-network-communications-traffic:
...
proxy server is a server (i.e., system or application) that acts as an
intermediary for clients who request system resources from
non-organizational or other organizational servers. System resources
that may be requested include files, connections, web pages, or
services. Client requests established through a connection to a proxy
server are assessed to manage complexity and provide additional
protection by limiting direct connectivity. Web content filtering
devices are one of the most common proxy servers that provide access to
the internet. Proxy servers can support the logging of Transmission
Control Protocol sessions and the blocking of specific Uniform Resource
Locators, Internet Protocol addresses, and domain names. Web proxies can
be configured with organization-defined lists of authorized and
unauthorized websites. Note that proxy servers may inhibit the use of
virtual private networks (VPNs) and create the potential for
"adversary-in-the-middle" attacks (also known as "man-in-the middle
...
| Solution Implementation                                                          |
+==================================================================================+
| The NILRT device is a component in the overall system. It is the responsibility  |
| of the system owner to ensure that network traffic is properly routed through    |
...
Limiting the number of external network connections facilitates the
monitoring of inbound and outbound communications traffic and is
important during transition periods from older to newer technologies.
Such transitions may require implementing older and newer technologies
simultaneously during the transition period and thus increase the number
of access points to the system.
...
+==================================================================================+
| System owners create programs to connect and send data from the NILRT device. It |
| is the responsibility of the system owner to ensure that the programming         |
| approach used limits the number of external network connections.                 |
+----------------------------------------------------------------------------------+
